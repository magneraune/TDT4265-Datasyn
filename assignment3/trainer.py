...
def compute_loss_and_accuracy(
    ...
    with torch.no_grad():
        for (X_batch, Y_batch) in dataloader:
            ...
            average_loss += loss
            output = output_probs.argmax(dim=1).squeeze()
            Y_batch = Y_batch.squeeze()

            #loss_avg += loss.item()
            #total_steps += 1
            accuracy += (output == Y_batch).sum().item()
            tot_img += output.shape[0]
            #outputs = torch.zeros_like(output_probs)
            #outputs[torch.arange(len(output_probs)), output_probs.argmax(1)] = 1
            #accuracy += torch.mean((outputs==Y_batch).all(0))
            steps += 1
            tot_img += Y_batch.size(0)
    average_loss = average_loss/steps
    accuracy = accuracy/tot_img
