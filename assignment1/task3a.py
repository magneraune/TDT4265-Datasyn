...
class SoftmaxModel:
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        z_exp = np.exp(np.dot(X,self.w))
        return z_exp / np.sum(z_exp,axis=1)[:,None]
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        grads = np.dot(X.T, outputs-targets)
        self.grad = (grads / targets.shape[0]) +2*self.l2_reg_lambda*self.w
